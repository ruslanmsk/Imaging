...
namespace ImageReadCS.task2
{
    public class CachedBicubicInterpolator
    {
        ...
        {
            _a00 = p[1][1];
            _a01 = -0.5 * p[1][0] + 0.5 * p[1][2];
            _a02 = p[1][0] - 2.5 * p[1][1] + 2 * p[1][2] - 0.5 * p[1][3];
            _a03 = -0.5 * p[1][0] + 1.5 * p[1][1] - 1.5 * p[1][2] + 0.5 * p[1][3];
            _a10 = -0.5 * p[0][1] + 0.5 * p[2][1];
            _a11 = 0.25 * p[0][0] - 0.25 * p[0][2] - 0.25 * p[2][0] + 0.25 * p[2][2];
            _a12 = -0.5 * p[0][0] + 1.25 * p[0][1] - p[0][2] + 0.25 * p[0][3] + 0.5 * p[2][0] - 1.25 * p[2][1] + p[2][2] - 0.25 * p[2][3];
            _a13 = 0.25 * p[0][0] - 0.75 * p[0][1] + .75 * p[0][2] - .25 * p[0][3] - .25 * p[2][0] + .75 * p[2][1] - .75 * p[2][2] + .25 * p[2][3];
